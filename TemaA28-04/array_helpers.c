...
unsigned int passengers_amount_in_airport (LayoverTable a, unsigned int h) {
  /* COMPLETAR */
  unsigned int total = 0u; //almacena la suma total de pasajeros en el aerop
  for(unsigned int i = 0; i < h-1; i++){ //bucle que itera sobre las horas
    total += a[i][0].passengers_amount; //suma la cant de pasajeros que llegan al aerop
    total -= a[i][1].passengers_amount; //resta la canti de pasajeros que salen del aerop
  }
  total += a[h][0].passengers_amount; //suma la cant de pasajeros que llegan al aerop en la hora h
...
void array_from_file(LayoverTable array, const char *filepath) {
  ...
  char code;
  //int i=0;
  /* COMPLETAR: lectura completa de todos los datos en el while*/
  while (!feof(file)) {
