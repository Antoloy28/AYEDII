...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        if ((k_year -FST_YEAR) >= YEARS){
            fprintf(stderr, "Invalid input for value of year, the value should be between 1980-2016..\n");
            exit(EXIT_FAILURE);
        }
        if((k_month-1u) >= 12){
    ...
    Weather weather = weather_from_file(file); //ya vania en el codigo este renglon.
